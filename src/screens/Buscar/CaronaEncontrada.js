...
function Options({navigation, route}) {
  ...
  function listenerBancoMotoristas() {
    ...
    database()
      .ref()
      .child(`${estado}/${cidade}/Motoristas`)
  ...
  async function getNomeMotorista(motoristaUID) {
    let reqs = await fetch(
      serverConfig.urlRootNode + `buscarUsuario/${motoristaUID}`,
      {
        method: 'GET',
        mode: 'cors',
        headers: {
  ...
  async function getPlacaCarroMotorista(motoristaUID) {
    let reqs = await fetch(
      serverConfig.urlRootNode + `buscarVeiculo/${motoristaUID}`,
      {
        method: 'GET',
        mode: 'cors',
        headers: {
  ...
  return (
    <SafeAreaView>
      ...
      <Image
        source={require('../../assets/images/carona-encontrada.png')}
      ...
      <ScrollView style={[styles.scrollView, {top: '5%'}]}>
        {vetorMotoristas.map(motorista => (
          <View style={styles.viewMotoristas} key={motorista.uid}>
            <Image
              source={{
                uri: motorista.url,
              }}
            ...
            <View style={{flexDirection: 'row'}}>
              <Text
                ...
                Classificação: {motorista.classificacao}
              </Text>
              <Icon
                name="star"
                size={18}
                color="#06444C"
                style={{alignSelf: 'center', left: '101%', top: '3%'}}
              />
            </View>
