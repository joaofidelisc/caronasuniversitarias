...
function Classificacao({navigation, route}) {
  ...
  const contarViagem = async () => {
    ...
    let reqs = await fetch(
      serverConfig.urlRootNode + `contarViagens/${uidMotorista}`,
      {
        method: 'GET',
        mode: 'cors',
        headers: {
  ...
  const retornaClassificacao = async () => {
    let reqs = await fetch(
      serverConfig.urlRootNode + `buscarUsuario/${uidMotorista}`,
      {
        method: 'GET',
        mode: 'cors',
        headers: {
    ...
    <SafeAreaView>
      ...
      <View
        ...
        <Text
          ...
          Feedback sobre como foi sua experiência durante a viagem.
        </Text>
        <TextInput
          style={{
            width: '75%',
