...
function ConfigurarCarona({navigation}) {
  ...
  async function defineCidadeEstado() {
    var response = await Geocoder.from(
    ...
    if (nomeDestino != '') {
      ...
      await EstadoApp.insertData({
        cidade: filtro_cidade[0].short_name,
        estado: filtro_estado[0].short_name,
  ...
  const receberInfoPassageiro = (estado, cidade) => {
    ...
    try {
      const events = new EventSource(
        `${serverConfig.urlRootNode}api/rabbit/obterInfo/passageiro/${estado}/Sao_Carlos`,
      );
  ...
  useEffect(() => {
    const recuperaEstadoApp = async () => {
      ...
      let ClassificarPassageiro = await AsyncStorage.getItem(
        'ClassificarPassageiro',
      );
      ...
      if (Oferecer != null && Oferecer != undefined) {
        ...
        ClassificarPassageiro != null &&
        ClassificarPassageiro != undefined
      ) {
  ...
  return (
    <SafeAreaView>
      ...
      <View
        ...
        <Image
          source={require('../../assets/images/duvidas-oferecer.png')}
          style={{
            height: 350,
            width: 350,
            alignSelf: 'center',
        ...
        <Text
          ...
          Quantas vagas você tem disponível em seu carro?
        </Text>
        <View
          style={{
            flexDirection: 'row',
