...
function Oferecer({route, navigation}) {
  ...
  function carregarInformacoes() {
    if (
      ...
      route.params?.destino == undefined
    ) {
      //buscar do banco
              ...
              if (vetorCaronistas.length == 0) {
                ...
                vetorCaronistas.some(caronista => {
                  if (caronista.uid === userSnapshot.key) {
                    vetorCaronistas[
                      vetorCaronistas.indexOf(caronista)
                    ].latitude = userSnapshot.val().latitudePassageiro;
                    vetorCaronistas[
                      vetorCaronistas.indexOf(caronista)
                    ].longitude = userSnapshot.val().longitudePassageiro;
  ...
  function getCaronistasMarker2() {
    try {
      const events = new EventSource(
        `${serverConfig.urlRootNode}getCaronistasMarker/${estado}/${cidade}/${vetorCaronistas}`,
      );
  ...
  */
  function oferecerCarona() {
    ...
    try {
      database()
        .ref(`${estado}/${cidade}/Passageiros/${uidPassageiro}`)
        .once('value')
        .then(snapshot => {
  ...
  */
  ...
  async function caronasAceitas2() {
    try {
      const events =
        new EventSource(`${serverConfig.urlRootNode}caronasAceitas/${estado}/${cidade}/${currentUser}/${oferecerMaisCaronas}/${vagasDisponiveis}
  ...
  const caronasAceitas = async () => {
    ...
    let arrayUIDs = [];
    const reference = database().ref(
      `${estado}/${cidade}/Motoristas/${currentUser}/caronasAceitas`,
    );
  ...
  */
  const buscarPassageiro = async (latitude, longitude, nome, uidCaronista) => {
    ...
    let distPassageiroMotorista = await distanciaPassageiroMotorista(
      latitude,
      longitude,
    );
    ...
    reference.once('value', function (snapshot) {
      ...
      if (
        distPassageiroMotorista < 6 &&
        !snapshot.val().caronistasAbordo.includes(uidCaronista)
      ) {
    ...
    reference_passageiro.once('value', function (snapshot) {
      ...
      if (!listaPassageirosAbordo.includes(uidPassageiro)) {
        if (listaPassageirosAbordo == '') {
          ...
          listaPassageirosAtualizada = listaPassageirosAbordo.concat(
            ', ',
            uidPassageiro,
          );
        }
      ...
      setArrayOfertasAceitas(
        arrayOfertasAceitas.filter(uid => uid != uidPassageiro),
      );
    });
  };
    ...
    <SafeAreaView>
      ...
      <View
            ...
            vetorCaronistas.map(caronista =>
              !passageiros.includes(caronista.uid) ? (
                <Marker
                  key={caronista.uid}
                  coordinate={{
                    latitude: caronista.latitude,
                    longitude: caronista.longitude,
                  }}
                  tappable={
                    caronista.caronasAceitas &&
                    currentUser &&
                    caronista.caronasAceitas.includes(currentUser)
                      ? true
                      : false
                  }
                ...
                >
                  <Image
                    source={
                      caronista.caronasAceitas == ''
                        ? require('../../assets/icons/caronista.png')
                        : caronista.caronasAceitas.includes(currentUser)
                        ? require('../../assets/icons/carona_aceita.png')
                        : require('../../assets/icons/caronista-nao-clicavel.png')
                    }
          ...
          {!oferecerMaisCaronas &&
            vetorCaronistas.map(caronista =>
              arrayOfertasAceitas.includes(caronista.uid) ? (
                ...
                >
                  <Image
                    source={require('../../assets/icons/carona_aceita.png')}
                    style={{width: width * 0.0742, height: width * 0.0742}}
                    resizeMode="contain"
                  />
                </Marker>
        ...
        {embarcarPassageiro && (
          <View
            ...
            <Text
              ...
              Passageiro(a) próximo!{'\n'}Assim que ele estiver no carro,
              ...
              {!oferecerMaisCaronas && alertaVagas && (
                <>
                  <Text
                    ...
                    Você atingiu o número máximo de caronistas!
                  </Text>
                  <Text
                    style={{
                      color: '#06444C',
                    ...
                    Para buscar um(a) passageiro(a), pressione uma vez no ícone
                    em verde e clique em buscar passageiro(a).
                  </Text>
                  <TouchableOpacity
                    style={{
                      backgroundColor: '#FF5F55',
          ...
          <View
            ...
            <View style={styles.modalView}>
              <Text
                ...
                Ainda tem vagas no seu veículo...
              </Text>
              <Text
                style={{
                  color: '#06444C',
                ...
                Tem certeza que deseja iniciar a viagem mesmo não tendo
        ...
        <Modal
          ...
          onRequestClose={() => {
            setModalBuscarPassageiro(!modalBuscarPassageiro);
          }}>
          <View
            style={{
              flex: 1,
          ...
          <View
            ...
            <View style={styles.modalView}>
              <Text
                ...
                Você não buscou todos os seus passageiros...
              </Text>
              <Text
                style={{
                  color: '#06444C',
                ...
                Busque os passageiros que aceitaram a sua carona para iniciar a
                viagem.
              </Text>
              <TouchableOpacity
                style={{
