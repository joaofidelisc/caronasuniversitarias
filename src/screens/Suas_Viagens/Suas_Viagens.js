...
function Suas_Viagens({route, navigation}) {
  ...
  const buscaChat = async secondUser => {
    let reqs = await fetch(
      serverConfig.urlRootNode + `buscaChat/${currentUser}/${secondUser}`,
      {
        method: 'GET',
        mode: 'cors',
        headers: {
    ...
    <SafeAreaView>
      ...
      <View
        ...
        {!existeViagem && !loading && (
          <>
            <Image
              source={require('../../assets/images/viagens-futuras.png')}
              style={{
                height: '40%',
                width: '70%',
                position: 'absolute',
          ...
          <ScrollView style={[styles.scrollView]}>
            ...
            {existeViagem &&
              ...
              arrayHistoricoViagens.map(viagem => (
                <View style={styles.viewViagens} key={viagem.refViagem}>
                  <Text
                    style={{
                      color: '#06444C',
                      fontWeight: '600',
                  ...
                  <Text
                    style={{
                      color: '#06444C',
                      fontWeight: '600',
                      fontSize: width * 0.9 * 0.06,
                      textAlign: 'center',
                      marginTop: 10,
                      marginLeft: width * 0.9 * 0.04,
                      marginRight: width * 0.9 * 0.04,
                    }}>
                    {viagem.nome}
                  </Text>
                  <Text
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      entrarEmContato(viagem.uidMotorista);
                    }}>
                    <Text
                      style={{
                        color: '#FF5F55',
                        fontWeight: '800',
                        fontSize: width * 0.9 * 0.04,
                        lineHeight: 24,
                        textAlign: 'center',
                      }}>
                      Entrar em contato
